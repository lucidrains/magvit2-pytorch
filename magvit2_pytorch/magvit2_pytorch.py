...
class VideoTokenizer(Module):
    ...
    def encode(
        ...
        if video_contains_first_frame:
            ...
            video_packed_shape = [torch.Size([i]) for i in [self.time_padding, 1, video_len - 1]]
        ...
        if encode_first_frame_separately:
            pad, first_frame, video = unpack(video, ff_packed_shape, 'b c * h w')
            first_frame = self.conv_in_first_frame(first_frame)

        video = self.conv_in(video)

        if encode_first_frame_separately:
            video, _ = pack([pad, first_frame, video], 'b c * h w')
